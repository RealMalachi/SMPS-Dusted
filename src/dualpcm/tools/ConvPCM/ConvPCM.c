...

{
	...
	if (ArgNumber <= 0x01)
	{
		...
		printf ("    to %s (signed wave fusion)\n\n", OutExt);
		printf (" -> This program will convert wave files of varying types into\n");
		printf ("    raw signed modular FE %s files, suitable for fusing quickly.\n\n", OutExt);
		printf ("    If a raw binary ROM is passed to this program, it shall be treated\n");
		printf ("    a an unsigned 8-bit mono sample, the rate of this sample shall not\n");
	...
	/*	if (strcmp (ExtName, InExt) != 0x00)
		...
		else if (OutputRate > InputRate)
		{
			...
			while (ChannelLoc < ChannelSize)
			{
				...
				AdvanceCur %= 1000;
				SampleA = ChannelL [ChannelLoc] << 0x0C;
				SampleB = ChannelL [++ChannelLoc] << 0x0C;
				SampDist = (SampleB - SampleA) / (ChanOutDest - ChanOutLoc);
				while (ChanOutLoc < ChanOutDest)
				{
					ChanOutL [ChanOutLoc++] = SampleA >> 0x0C;
			...
			while (ChannelLoc < ChannelSize)
			{
				...
				ChanOutDest += (AdvanceCur / 1000);
				AdvanceCur %= 1000;
				SampleA = ChannelR [ChannelLoc] << 0x0C;
				SampleB = ChannelR [++ChannelLoc] << 0x0C;
				SampDist = (SampleB - SampleA) / (ChanOutDest - ChanOutLoc);
				while (ChanOutLoc < ChanOutDest)
				{
					ChanOutR [ChanOutLoc++] = SampleA >> 0x0C;
					if (Interpolation == TRUE)
					{
						SampleA += SampDist;
					}
		...
		else
		{
			...
			while (ChannelLoc < ChannelSize)
			{
				...
				AdvanceCur %= 1000;
				SampleA = ChannelL [ChannelLoc];
				SampleB = SampleA;
				for (DivCount = 1; ChannelLoc < ChannelDest; DivCount++)
				{
					SampleB += ChannelL [ChannelLoc++];
				}
				if (Interpolation == TRUE)
				{
					SampleB /= DivCount;
					ChanOutL [ChanOutLoc++] = SampleB;	// Interpolated
				}
				else
				{
					ChanOutL [ChanOutLoc++] = SampleA;	// Linear
				}
			}
		...
		int OutputSize = ChanOutSize + (0x20 - (ChanOutSize & 0x1F));
		...
		for (OutputLoc = 0x00, ChanOutLoc = 0x00; ChanOutLoc < ChanOutSize; ChanOutLoc++)
		{
			...
			Output [OutputLoc++] = (SampleA << 0x18) >> (0x18 + 0x01);
		}
		while ((OutputLoc & 0x1F) != 0x1F)
		{
			Output [OutputLoc++] = 0x00;
		}
		Output [OutputLoc++] = 0x80;
