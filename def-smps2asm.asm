...
smpsHeaderPSG macro loc,pitch,vol,mod,voice
	...
	elseif SourceDriver>=3
	dc.b	pitch,(((vol)<<3)&$7F)|((vol)&$80),mod,voice
	else
	dc.b	(pitch+psgdelta)&$FF,(((vol)<<3)&$7F)|((vol)&$80),0,voice
	endif
	endm
;		if (mod<>0) && (MOMPASS=1)
...
;		endif
	...
	elseif SourceDriver>=3
	dc.b	chanid,pitch,(((vol)<<3)&$7F)|((vol)&$80),loc-((*)+1+3)
	else
	dc.b	chanid,(pitch+psgdelta)&$FF,(((vol)<<3)&$7F)|((vol)&$80),loc-((*)+1+3)
	endif
	endm
; ---------------------------------------------------------------------------
...
smpsPSGSetVol macro vol
	dc.b	cVolSetPSG,(((vol)<<3)&$7F)|((vol)&$80)
	endm
; Add to Volume by xx
...
smpsFMVolEnv macro voice,mask
	if mask&$F<>$F
	fatal "this driver doesn't support smpsFMVolEnv per-operator enveloping, please set the mask to $F"
	else
	dc.b	cVolEnv,voice
	endif
	endm
...
; in smps-dusted, val defines the fadein counter, if not defined then use the default
...
; - anything aside for that 
; fyi, smps-z80 cxSongFadeIn uses the same byte as smps-68000 cxCommunicate
; essentially, all of S3Ks usages were erroneously unchanged communication attempts leftover from Sonic 1 and 2
; TODO: valid to control the type of fade-in further if need be
